...
namespace Fit_Growth_Curves
{
    ...
    public partial class SelectablePlateMap : UserControl
    {
        ...
        protected PlateHeatMap.PlateType plateType = PlateHeatMap.PlateType.WELL48;
        ...
        public void SwitchToNewPlateType(PlateHeatMap.PlateType type)
        {
            plateType = type;
            InitializeLayout();
        ...
        {
            if (plateType==PlateHeatMap.PlateType.WELL48)
            {
                NumRows = 6;
                NumCols = 8;
                NumWells = 48;
        ...
        public void AssignAllWellsToGroup(int GroupIndex)
        {
            for (int i = 0; i < CurWellAssignment.Length; i++)
            {
                CurWellAssignment[i] = GroupIndex;
            }
            RecreateImage();
        }
        ...
        {
            if (CellNameToInts.ContainsKey(Well))
            {
        ...
        public IEnumerable<string> GetNamesOfAllAssignedWells()
        {
            List<int> Indexes = new List<int>();
            for (int i = 0; i < CurWellAssignment.Length; i++)
            { if (CurWellAssignment[i] >0)Indexes.Add(i); }
            var Names = from x in Indexes select IntsToCellName[x];
            return Names;
        }
        ...
        private void SelectablePlateMap_SizeChanged(object sender, EventArgs e)
        {
            InitializeLayout();
        }
    }
