        ...
        void  ChartPickData_ZoomEvent(ZedGraphControl sender, ZoomState oldState, ZoomState newState)
{
 	if (ChartPickDataLastFitXValue != BAD_DATA_VALUE)
        ...
        private void rbtnPlateOptions_CheckedChanged(object sender, EventArgs e)
        {
            ...
            else if (sender == rbtnGroupGrowthRate)
            {
                curPlateValueFunction = (x) => GroupFit.WellParameters[x.DataSetName].GrowthRate;
            }
            else if (sender == rbtnRS)
            {
                curPlateValueFunction = new PlateHeatMap.GrowthCurveDoubleValueGetter((x) => x.GrowthRate.R2);
            }
            ...
            else if (sender == rbtnPlateQuadR2)
            {
                curPlateValueFunction = new PlateHeatMap.GrowthCurveDoubleValueGetter((x) => x.QuadModel.R2);
            }
            else if (sender == rbtnPlotQuadraticRate)
            {
                curPlateValueFunction = new PlateHeatMap.GrowthCurveDoubleValueGetter((x) => x.QuadModel.GrowthRateAtValue(0.15));
            }
        ...
        private void lstGroupModelGroups_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (lstGroupModelGroups.SelectedIndex > -1)
            {
                selectPlatesForGroupModel.CurGroupToSelect = lstGroupModelGroups.SelectedIndex;
            }
        }
        private void rbtnGM_CheckedChanged(object sender, EventArgs e)
        {

            RadioButton btn = sender as RadioButton;
            if (btn.Checked)
            {
                RemakeTreatmentGraphGroupedData();
            }
        }
        private void btnFitGroup_Click(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;
            try
            {
                GCC.ForEach(x => x.GroupFit = null);
                var i = selectPlatesForGroupModel.GetNamesOfAllAssignedWells();
                var growthCurves = from x in GCC where i.Contains(x.DataSetName) select x;
                GrowthCurveCollection newGCC = new GrowthCurveCollection();
                newGCC.AddRange(growthCurves);
                if (newGCC.Count == 0)
                {
                    this.Cursor = Cursors.Default;
                    MessageBox.Show("No wells selected");
                }
                else
                {
                    GroupFit = new GroupFitter(newGCC);
                }
                RemakeTreatmentGraphGroupedData();
            }
            catch (Exception thrown)
        ...
        private void btnCopyGroupAssignments_Click(object sender, EventArgs e)
        {
            for(int i=0;i<SelectablePlateMap.MAX_GROUP_ASSIGNMENTS;i++)
            {
                Color o;
                selectablePlateMap1.GetNamesOfWellsAssignedToGroup(i, out o).ToList().ForEach(x=>selectPlatesForGroupModel.AssignWellToGroup(x,i));
            }
            selectPlatesForGroupModel.RecreateImage();
            selectPlatesForGroupModel.Refresh();
            selectPlatesForGroupModel.Invalidate();
            RemakeTreatmentGraph();
        }
        private void selectPlatesForGroupModel_GroupsChanged(object sender, EventArgs e)
        {
            RemakeTreatmentGraphGroupedData();
        }
        ...
        private void selectablePlateMap1_GroupsChanged_1(object sender, EventArgs e)
        {
            RemakeTreatmentGraphGroupedData();
        }
        ...
        private void SelectFileToOpen(bool useNumeric=false)
        {
            ClearLists();
            GCC.Clear();
            ...
            if (openFileDialog1.FileName.Length > 0)
            {
                ...
                try
                {
                    if (!useNumeric)
                    {
                        var data = ImportDelimitedDataFile.ImportDateTimeCSV(openFileDialog1.FileName);
                        AddGrowthCurves(data);
                    }
                    else {
                        var data = ImportDelimitedDataFile.ImportNumericTimeCSV(openFileDialog1.FileName);
                        AddGrowthCurves(data);
                    }
                }
